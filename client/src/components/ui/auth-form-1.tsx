...
function AuthSocialButtons({ isLoading }: { isLoading: boolean }) {
  const handleGoogleLogin = () => {
    // Redirect to Spring Boot OAuth2 endpoint — backend handles the rest
...
function AuthSignIn({ onForgotPassword, onSignUp }: AuthSignInProps) {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    ...
    try {
      // Logic moved to AuthContext.login to handle cookies + state + redirect in one go
      await login(data);
    } catch (err: unknown) {
      const msg =
...
function AuthSignUp({ onSignIn }: AuthSignUpProps) {
  ...
  const onSubmit = async (data: SignUpFormValues) => {
    setFormState((prev) => ({ ...prev, isLoading: true, error: null }));
    try {
      await api.post("/api/auth/v1/register", {
        email: data.email,
        password: data.password,
      });
      // After register, check onboarding
      navigate("/onboarding");
    } catch (err: unknown) {
      const msg =
        (err as { response?: { data?: { message?: string } } })?.response?.data
...
function AuthForgotPassword({ onSignIn, onSuccess }: AuthForgotPasswordProps) {
  ...
  const onSubmit = async (_data: ForgotPasswordFormValues) => {
    setFormState((prev) => ({ ...prev, isLoading: true, error: null }));
    // No backend endpoint for forgot-password — show placeholder
    setTimeout(() => {
      setFormState((prev) => ({ ...prev, isLoading: false }));
      onSuccess();
    }, 800);
...
function AuthResetSuccess({ onSignIn }: { onSignIn: () => void }) {
  return (
    ...
    >
      ...
      <Button type="button" className="mt-8 w-full" onClick={onSignIn}>
        Back to sign in
      </Button>
    </motion.div>
  );
